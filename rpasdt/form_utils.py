...
GraphTypeToFormFieldsConfigMap = {
    GraphTypeEnum.WATTS_STROGATZ: {
        'n': FormFieldConfig(field_name='n', default_value=100,
                             type=FieldInputType.INTEGER),
        'k': FormFieldConfig(field_name='k', default_value=4,
                             type=FieldInputType.INTEGER),
        'p': FormFieldConfig(field_name='p', default_value=0.5,
                             type=FieldInputType.DOUBLE),
    },
    GraphTypeEnum.BALANCED_TREE: {
        'r': FormFieldConfig(field_name='r', default_value=2,
                             type=FieldInputType.INTEGER,
                             help_text='Branching factor of the tree; each node will have r children'),
        'h': FormFieldConfig(field_name='h', default_value=5,
                             type=FieldInputType.INTEGER,
                             help_text='Height of the tree'),
    },
    GraphTypeEnum.COMPLETE: {
        'n': FormFieldConfig(field_name='n', default_value=5,
                             type=FieldInputType.INTEGER,
    ...
    GraphTypeEnum.ERDOS_RENYI: {
        ...
        'seed': FormFieldConfig(field_name='seed', default_value=100,
                                type=FieldInputType.INTEGER,
                                help_text='Indicator of random number generation state'),

    },
    GraphTypeEnum.CAVEMAN_GRAPH: {
        'l': FormFieldConfig(field_name='l', default_value=5,
                             type=FieldInputType.INTEGER,
                             help_text='Cliques number'),
        'k': FormFieldConfig(field_name='k', default_value=3,
                             type=FieldInputType.INTEGER,
...
NodeAttributeFormFieldsConfig = {
    NodeAttributeEnum.COLOR: FormFieldConfig(
        field_name=NodeAttributeEnum.COLOR, default_value=NODE_COLOR,
        type=FieldInputType.COLOR, label='Node color'),
    NodeAttributeEnum.SIZE: FormFieldConfig(field_name=NodeAttributeEnum.SIZE,
                                            default_value=NODE_SIZE,
                                            type=FieldInputType.INTEGER,
                                            label='Node size'),
    NodeAttributeEnum.EXTRA_LABEL: FormFieldConfig(
        field_name=NodeAttributeEnum.EXTRA_LABEL,
        type=FieldInputType.SINGLE_TEXT, label='Node extra label'),
    NodeAttributeEnum.LABEL: FormFieldConfig(
        field_name=NodeAttributeEnum.LABEL,
        type=FieldInputType.SINGLE_TEXT, label='Node name'),
    NodeAttributeEnum.SOURCE: FormFieldConfig(
        field_name=NodeAttributeEnum.SOURCE,
        type=FieldInputType.CHECKBOX, label='Is source ?')
...
def get_graph_form_config(graph_type: GraphTypeEnum):
    return DynamicFormConfig(
        field_config=GraphTypeToFormFieldsConfigMap[
            graph_type],
...
def get_node_edit_config(node_index: int):
    return DynamicFormConfig(
        title=f'Edit node {node_index}',
        field_config=NodeAttributeFormFieldsConfig
    )


def get_diffusion_model_fields_config(diffusion_model: DiffusionModel) -> Dict[
    str, FormFieldConfig]:
    ...
    for field, details in parameters.items():
        ...
        default_val = details.get("default", random.uniform(range[0], range[
            1]) if range else 0)
        result[field] = FormFieldConfig(field_name=field, range=range,
                                        type=FieldInputType.DOUBLE,
                                        default_value=default_val)
    return result


def get_diffusion_model_form_config(diffusion_model: DiffusionModel,
                                    title: str = None):
    if not title:
        title = f'Edit {diffusion_model.name} diffusion model params'
    return DynamicFormConfig(
        title=title,
        field_config=get_diffusion_model_fields_config(
