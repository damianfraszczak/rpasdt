...
from rpasdt.controller.controllers import CommunityAnalysisControllerMixin, \
...
class SourceDetectionGraphController(GraphController,
                                     CentralityAnalysisControllerMixin,
    ...
    def mark_nodes(self):
        detected_sources = self.source_detector.detected_sources if isinstance(
            self.source_detector.detected_sources, list) else [
            self.source_detector.detected_sources]
        for estimated_node in detected_sources:
            self.graph.nodes[estimated_node][
                NodeAttributeEnum.COLOR] = self.experiment.graph_config.estimated_source_node_color
