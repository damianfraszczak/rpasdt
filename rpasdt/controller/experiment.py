...
from rpasdt.controller.controllers import CentralityAnalysisControllerMixin, CommunityAnalysisControllerMixin
...
from rpasdt.network.networkx_utils import get_graph_form_field_config, \
    set_node_attributes, show_graph_config_dialog


class ExperimentGraphController(GraphController, CentralityAnalysisControllerMixin, CommunityAnalysisControllerMixin):
    ...
    def reload_experiment(self, experiment: Experiment):
        if experiment:
            run_long_task(
                function=load_graph,
                function_kwargs={'graph_type': experiment.graph_type,
    ...
    def handler_edit_experiment(self):
        self.reload_experiment(show_dynamic_dialog(
    ...
    def handler_edit_graph_type_properties(self):
        ...
        if graph_type_properties and graph_type_properties != self.experiment.graph_type_properties:
    ...
    def handler_create_diffusion(self):
        experiment = show_dynamic_dialog(object=DiffusionExperiment(
            source_graph=self.graph,
            graph_config=self.graph_config),
            title='Create a new diffusion experiment'
        )
        if experiment:
            self.window.show_diffusion_window(experiment=experiment, controller=DiffusionGraphController(
                window=self.window,
                experiment=experiment
            ))
    ...
    def handler_clear_sources(self):
        set_node_attributes(self.graph, [
            ...
            NodeAttribute(NodeAttributeEnum.COLOR, self.graph_config.susceptible_node_color),
        ])
        self.redraw_graph()
    ...
    def handler_select_sources(self):
        ...
        if sources_config:
            ...
            set_node_attributes(self.graph, [
                ...
                NodeAttribute(NodeAttributeEnum.COLOR, self.graph_config.source_node_color),
            ], nodes_list=sources)
            self.redraw_graph()
