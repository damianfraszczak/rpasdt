...
def get_leaves(tree: Graph):
    return [x for x in tree.nodes() if
            tree.out_degree(x) == 0 and tree.in_degree(x) == 1]


def children_processed(tree: Graph, node: int,
                       processed_nodes: List[int]) -> bool:
    children = get_children(tree, node)
    return len(children) == 0 or all(
        child in processed_nodes for child in children)


class RumorCenterCommunityBasedSourceDetector(
    ...
    def find_sources_in_community(self, graph: Graph):
        ...
        for sourceNode in nodes:
            ...
            while not children_processed(bfs_tree, sourceNode,
                                         list(messages_down.keys())):
                for node in nodes:
                    if node in leaves:
                        messages_up[node] = 1
                        messages_down[node] = 1
                    elif node != sourceNode:
                        if children_processed(bfs_tree, node,
                                              list(messages_down.keys())):
