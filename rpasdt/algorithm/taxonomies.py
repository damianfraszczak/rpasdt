...
class CentralityOptionEnum(StringChoiceEnum):
    ...
    EDGE_BETWEENNESS = (
        'edge_betweenness', ('Compute betweenness centrality for edges.'))
    HARMONIC = ('harmonic', 'Compute harmonic centrality for nodes.')
    PERLOCATION = (
        'percolation', 'Compute the percolation centrality for nodes.')
...
class CommunityOptionEnum(StringChoiceEnum):
    ...
    K_CLIQUE = (
        'k_clique', 'Finds communities in a graph using the K-Clique method.')
    ...
    K_CORONA = (
        'k_corona', 'Finds korona in a graph using the K-shell method.')
    K_MEANS = (
        'k_means', 'Finds communities in a graph using the K-means method.')


class SourceSelectionOptionEnum(StringChoiceEnum):
    RANDOM = ('random', 'Select sources in random way.')
    DEGREE = (
        'degree', 'Select sources according to degree centrality metrics.')
    CLOSENESS = (
        'closeness',
...
class SourceDetectionAlgorithm(StringChoiceEnum):
    ...
    RUMOR_CENTER = (
        'rumor_center', 'Find sources with Rumor center algorithm.')
    CENTRALITY_BASED = (
        'centrality', 'Find sources with centrality based algorithm.')
