...
from rpasdt.form_utils import get_diffusion_model_fields_config, \
...
def get_graph_form_field_config(graph_type: GraphTypeEnum) -> Dict[
    str, FormFieldConfig]:
...
def get_graph_default_properties(graph_type: GraphTypeEnum) -> Dict[str, Any]:
    return {field_name: field_config.default_value for field_name, field_config
            in
            get_graph_form_field_config(graph_type).items()}


def get_diffusion_model_default_properties(diffusion_model: DiffusionModel) -> \
    Dict[str, Any]:
    return {field_name: field_config.default_value for field_name, field_config
            in
            get_diffusion_model_fields_config(diffusion_model).items()}


def create_node_network_dict(graph: Graph, data_key: str,
                             default_val: Optional[Any] = None,
                             skip_empty: bool = False):
    return {n: graph.nodes[n].get(data_key, default_val) or (
        '' if skip_empty else n) for n in graph.nodes}


def create_node_network_array(graph: Graph, data_key: str,
                              default_val: Optional[Any] = None):
    return [graph.nodes[n].get(data_key, default_val) or n for n in
            graph.nodes]


def map_networkx_communities_to_dict(communities: Tuple[Set[int]]) -> Dict[
    int, int]:
    return {index + 1: community for index, community in
            enumerate(map(sorted, communities))}
...
def get_nodes_color(nodes_value: List[int], cmap=None):
    ...
    return [mapper.to_rgba(i)
            for i in nodes_value]


def set_node_attributes(graph: Graph, attributes: List[NodeAttribute],
                        nodes_list=None):
...
def show_graph_config_dialog(graph_type: GraphTypeEnum,
