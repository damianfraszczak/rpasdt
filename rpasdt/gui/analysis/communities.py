...
from rpasdt.network.networkx_utils import get_nodes_color, get_community_index
...
class CommunityGraphPanel(NetworkxGraphPanel):
    ...
    def draw_nodes(self):
        return nx.draw_networkx_nodes(self.graph, self.graph_position, node_size=250,
                                      node_color=[self.communities_color[get_community_index(community)] for community in
                                                  self.data.node_values],
                                      nodelist=self.data.node_list)

    def draw_legend(self):
        legend_elements = [
            Line2D([0], [0], marker='o', color='w', label=f'Community {community}',
                   markerfacecolor=self.communities_color[get_community_index(community)], markersize=15) for community
            in self.data.communities_list
